...
const addTeamsPost = (URL, username, teamsToAdd, skipValidation) => {
    ...
    let validationBit = skipValidation ? 1 : 0;
    apiHeaders.headers.skipValidation = validationBit;

    return axios.post(`${URL}/user/${username}/addteams`, {
    ...
    return new Promise((resolve, reject) =>{
        request(`https://www.hltv.org/search?query=${team}`, (error, response, html) => {
            if (!error && response.statusCode === 200) {
                ...
                if (teamsDiv && teamsDiv.length && teamsDiv[0].attribs && teamsDiv[0].attribs.title && teamsDiv[0].attribs.title.toString().toUpperCase() === team.toString().toUpperCase()) {
