	...
	request('https://www.hltv.org/matches', (error, response, html) => {
		if (!error && response.statusCode === 200) {
			...
			while (numMatches > iteration) {
				...
				if (minutesLeft > 5 || hoursLeft > 0) {
					// if there are no matches in the next 5 minutes, exit the loop
					console.log(
						'Because this match starts in more than 5 minutes, stop and scrape again in 5 minutes'
					);
					...
					let users = [];
					axios
						.get(`${URL}/teams/${team1}/getusers`, apiHeaders)
						.then(function (response) {
							...
							console.log(
								`These are the users subscribed to ${team1}: ${JSON.stringify(
									users
								)}`
							);
						})
					...
					axios
						.get(`${URL}/teams/${team2}/getusers`, apiHeaders)
						.then(function (response) {
							// handle success
							console.log(response.data);
							...
							console.log(
								`These are the users subscribed to ${team1} and ${team2}: ${JSON.stringify(
									users
								)}`
							);
